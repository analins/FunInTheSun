...
function performLogic(req, res, callback) {
    // Fire it up!
    getCityData(req);

    // Send back the result of our findings
    res.json({cities: citiesArr});

}

function getCityData (req) {
    ...
    //************ request url still needs structuring ***************
    for (var i = 0; i < dbCities.length; i++) {
        request(urlAndQueryAndAPIKey, function(err, response, body){ //url + '/q/' + process.env.APIKEY + "/" + state + "/" + city
            citiesArr.push(JSON.parse(body));
        });
    }

    //Once you have the data stored in cities Arr go call the scoreLogic function
    return scoreLogic();
}
...
function scoreLogic (){
    ...
    for (var i = 0; i < citiesArr.length; i++) {
        var chosenDay = parseInt(Math.random()*10); // ***** Number of days between today and desired travel day [min:0, max:9] **********
...
// Helper functions ---------------------------------------------------
function temperature (city, attr){
    return city.score += parseInt(attr.high.fahrenheit);
}

function percipChance (city, attr){
    var percentChance =  (1 - attr.pop/100) * 20 - 10; //range -10 to 10
    return city.score += percentChance;
}

function percipAmount (city, attr){
    var amount = (attr.qpf_day.in * 10) + (attr.snow_day.in * 10); //Add daytime rain and snow accumulation
    return city.score -= amount; //subtract from score
}

function conditions (city, attr){
...
function wind (city, attr){
    var windSpeed = attr.avewind.mph;
    return city.score -= windSpeed;
}
