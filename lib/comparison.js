...
function getCityData (req) {
    ...
    for (var i = 0; i < dbCities.length; i++) {
        ...
        $.ajax({  //********* Server Side. No ajax here... will have to get Server module *****************
            method: 'get',
            url: comment,//url + '/q/' + process.env.APIKEY + "/" + state + "/" + city
            success: addCityData //function to add data to citiesArr
        });
    }
...
function pop (city, attr){
    var positiveChance = (1 - attr.pop) * 100;
    city.score *= positiveChance;
}
